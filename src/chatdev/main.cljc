(ns chatdev.main
  "Kotodama WASM entrypoint for the Chatdev clean-room actor (L4) — Clojure port.
  ...
  facts (`chatdev.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "chatdev")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"chatdev.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "chatdev-compat" :tier tier :entities entities} 200])
...
(defn start! [] :chatdev-compat/ready)
